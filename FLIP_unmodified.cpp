...
void image::computeFeatureDifference(image& refImage, image& testImage, const float ppd)
{
    ...
    std::ofstream edge,point;
    edge.open("edge");
    point.open("point");
    for(int i = 0; i< edgeFilters.mHeight;i++)
    {
        for(int j=0; j<edgeFilters.mWidth;j++)
        {
            color3 e=edgeFilters.get(i,j);
            std::string s=std::to_string(e.x) + 
                        ","+std::to_string(e.y) + 
                        ","+std::to_string(e.z);
            if(j<=edgeFilters.mWidth)
                s+="\t";
            
            edge << s;

            color3 p=pointFilters.get(i,j);
            std::string sp=std::to_string(p.x) + 
                        ","+std::to_string(p.y) + 
                        ","+std::to_string(p.z);
            if(j<=edgeFilters.mWidth)
                sp+="\t";
    ...
    edgeFilters.save("edge.png");
    pointFilters.save("point.png");
...
void image::computeFLIPError(image& refImage, image& testImage, bool verbose)
{
    ...
    std::cout << "transform YCxCz: " << transform_YCxCz.count() << "s\n";
    std::cout << "preprocess: " << preprocess.count() << "s\n";
    std::cout << "colorDiff: " << colorDiff.count() << "s\n";
    std::cout << "featureDiff: " << featureDiff.count() << "s\n";
    std::cout << "finalError: " << finalError.count() << "s\n";
}
